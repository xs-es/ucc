...
class TestQHRFLogicalEquivalence:
    ...
    def test_cnot_cancellation_with_equivalence(self):
        ...
        print(
            f"✅ CNOT cancellation: {original_cnots} → {optimized_cnots} with equivalence preserved"
        )
    ...
    def test_measurement_statistics_preservation(self):
        ...
        print(
            "✅ Measurement statistics preserved (actual quantum statistics)"
        )
    ...
    def test_three_qubit_ghz_state_preservation(self):
        ...
        for i in [1, 2, 3, 4, 5, 6]:  # All states except |000⟩ and |111⟩
            assert abs(probs_orig[i]) < 1e-10, (
                f"Original GHZ contaminated with |{i:03b}⟩"
            )
            assert abs(probs_opt[i]) < 1e-10, (
                f"Optimized GHZ contaminated with |{i:03b}⟩"
            )
    ...
    def test_circuit_with_measurements_preserved(self):
        ...
        print(
            "✅ Circuit with measurements - structure and equivalence preserved"
        )
...
class TestUCCBenchmarkCompatibility:
    ...
    def test_gate_count_reduction_with_equivalence(self):
        ...
        ucc_improved = (
            gate_improvement or twoq_improvement or depth_improvement
        )
        ...
        if ucc_improved:
            ...
            print(
                "⚠️  No UCC improvement detected - may need circuit adjustments"
            )
...
def test_comprehensive_collaborator_response():
    ...
    print("=" * 80)
    print(
        "@bachase: 'sv1 = Statevector(circuit); sv2 = Statevector(optimized_circuit); assert sv1.equiv(sv2)'"
    )
    print(
        "@jordandsullivan: 'not checking logical equivalence of circuits, which is critical'"
    )
    print(
        "@jordandsullivan: 'statistics means measurement statistics, not gate counts'"
    )
    print("=" * 80)
    ...
    tests = [
        (
            "Bell State Preservation (Critical)",
            test_instance.test_bell_state_preservation_critical,
        ),
        (
            "CNOT Cancellation with Equivalence",
            test_instance.test_cnot_cancellation_with_equivalence,
        ),
        (
            "Measurement Statistics (Real Quantum Stats)",
            test_instance.test_measurement_statistics_preservation,
        ),
        (
            "Three-Qubit GHZ Preservation",
            test_instance.test_three_qubit_ghz_state_preservation,
        ),
        ("No False Optimizations", test_instance.test_no_false_optimizations),
        (
            "Operator Equivalence",
            test_instance.test_operator_equivalence_verification,
        ),
        (
            "Circuits with Measurements",
            test_instance.test_circuit_with_measurements_preserved,
        ),
        (
            "UCC Benchmark with Equivalence",
            ucc_instance.test_gate_count_reduction_with_equivalence,
        ),
    ]
