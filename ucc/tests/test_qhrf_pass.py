...
try:
    from ucc.transpilers.qhrf_pass import (
...
class TestQHRFHarmonicResonancePass:
    ...
    def test_cnot_elimination_simple(self):
        ...
        assert optimized_cnots == 0, (
            f"QHRF should eliminate all CNOT gates, but found {optimized_cnots}"
        )
    ...
    def test_cnot_elimination_multiple(self):
        ...
        optimized_cnots = optimized_circuit.count_ops().get("cx", 0)
        assert optimized_cnots == 0, (
            f"QHRF should eliminate ALL CNOT gates, found {optimized_cnots}"
        )
    ...
    def test_logical_structure_preservation(self):
        ...
        assert "cx" not in ops or ops.get("cx", 0) == 0, (
    ...
    def test_qhrf_pattern_structure(self):
        ...
        assert ops.get("cx", 0) == 0, (
            "QHRF pattern should contain NO CNOT gates"
        )
    ...
    def test_coherence_stabilization_enabled(self):
        ...
        qhrf_pass = QHRFHarmonicResonancePass(
            apply_coherence_stabilization=True
        )
        dag = circuit_to_dag(circuit)
        optimized_dag = qhrf_pass.run(dag)
        optimized_circuit = dag_to_circuit(optimized_dag)
        ...
        assert final_cnots == 0, (
            "Should have zero CNOTs after QHRF optimization"
        )
    ...
    def test_coherence_stabilization_disabled(self):
        ...
        qhrf_pass = QHRFHarmonicResonancePass(
            apply_coherence_stabilization=False
        )
        dag = circuit_to_dag(circuit)
        optimized_dag = qhrf_pass.run(dag)
        optimized_circuit = dag_to_circuit(optimized_dag)
        ...
        final_cnots = optimized_circuit.count_ops().get("cx", 0)
        assert final_cnots == 0, (
            "Should eliminate all CNOTs even without coherence stabilization"
        )
    ...
    def test_pass_manager_integration(self):
        ...
        optimized_cnots = optimized_circuit.count_ops().get("cx", 0)
        assert optimized_cnots == 0, (
            "PassManager should enable complete CNOT elimination"
        )
        assert optimized_circuit.size() > circuit.size(), (
            "Should add QHRF gates"
        )
    ...
    def test_statistics_accuracy(self):
        ...
        assert stats["cnot_gates_eliminated"] == 3, (
            "Should eliminate exactly 3 CNOTs"
        )
        assert stats["resonance_patterns_applied"] == 3, (
            "Should apply 3 resonance patterns"
        )
        assert stats["coherence_operations_added"] >= 0, (
            "Coherence ops should be non-negative"
        )
    ...
    def test_complex_circuit_optimization(self):
        ...
        final_cnots = optimized_circuit.count_ops().get("cx", 0)
        assert final_cnots == 0, (
            f"All CNOTs should be eliminated, but found {final_cnots}"
        )
...
class TestQHRFPerformanceMetrics:
    ...
    def test_two_qubit_gate_reduction_metric(self):
        ...
        assert reduction == 4, (
            f"Should reduce 4 two-qubit gates, reduced {reduction}"
        )
    ...
    def test_runtime_performance(self):
        ...
        assert optimization_time < 1.0, (
            f"Optimization took {optimization_time:.3f}s, should be < 1s"
        )
